...
//         "https://cors-anywhere.herokuapp.com/http://gpbased.somee.com/api/Accounts/Register",
//         values
//       );
...
const SignUp = () => {
  ...
  const validationSchema = Yup.object({
    firstName: Yup.string()
      .matches(/^[A-Za-z]+$/, 'First Name must only contain letters')
      .required('First Name is required')
      .test('capitalize', 'First Name must start with a capital letter', value => /^[A-Z]/.test(value)),
    lastName: Yup.string()
      .matches(/^[A-Za-z]+$/, 'Last Name must only contain letters')
      .required('Last Name is required')
      .test('capitalize', 'Last Name must start with a capital letter', value => /^[A-Z]/.test(value)),
    username: Yup.string()
      .matches(/^[A-Za-z0-9]+$/, 'Username must only contain letters and digits')
      .required('Username is required')
      .test('no-spaces', 'Username can\'t contain spaces', value => !/\s/.test(value)),
    email: Yup.string()
      .email('Invalid email format')
      .required('Email is required'),
    phoneNumber: Yup.string()
      .matches(/^01[0125][0-9]{8}$/, 'Phone number must be 11 digits, start with 01 and second digit 0, 1, or 2')
      .required('Phone Number is required'),
    gender: Yup.string()
    ...
    password: Yup.string()
      .min(6, 'Password must be at least 6 characters')
      .matches(/^(?=.*[a-z])(?=.*\d)(?=.*[^A-Za-z0-9])/, 'Password must contain at least one lowercase letter, one digit, and one special character')
      .required('Password is required'),
    confirmPassword: Yup.string()
      .oneOf([Yup.ref('password'), null], 'Passwords must match')
      .required('Confirm Password is required'),
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      ...
      const response = await axios.post('http://localhost:5132/api/Accounts/Register', payload);
      console.log('Sign-Up Success:', response.data);
      alert('Sign-Up Successful');
      resetForm();
      navigate('/signin');
  ...
  return (
    ...
    <div>
              ...
              <Form>
                ...
                <div className="form-group">
                  <Field as="select" name="role" className="select-field">
                    ...
                    <option value="Farmer">Farmer</option>
                    <option value="Supplier">Supplier</option>
                    <option value="AgriculturalExpert">Expert</option>
                    <option value="SystemAdministrator">Admin</option>
                  </Field>
                  <ErrorMessage name="role" component="div" className="error" />
                </div>
      ...
      <div className=' login-here-div'>
        <p className=' m-auto p-3'>Already Have An Account?
          <Link to="/signin"> Login here</Link>
        </p>
      </div>
